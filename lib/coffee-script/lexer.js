...
(function() {
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.lineToken = function() {
      ...
      if (size - this.indebt === this.indent || size + this.outdebt === this.indent) {
        if (size - this.indebt === this.indent) {
          this.outdebt = 0;
        }
        if (size + this.outdebt === this.indent) {
          this.indebt = 0;
        }
        if (noNewlines) {
      ...
      if (size > this.indent) {
        ...
        if (this.indebt) {
          this.oldDebts.push(this.indebt);
        } else if (this.outdebt) {
          this.oldDebts.push(-this.outdebt);
        } else {
          this.oldDebts.push(0);
        }
    ...
    Lexer.prototype.outdentToken = function(moveOut, noNewlines, outdentLength) {
      var indent, oldDebt;
      while (moveOut > 0) {
        indent = last(this.indents);
        oldDebt = last(this.oldDebts);
        if (indent == null) {
          break;
        }
        if (indent > moveOut && (!(oldDebt > 0) || indent - moveOut !== oldDebt)) {
          break;
        }
        this.indents.pop();
        this.oldDebts.pop();
        ...
        this.indent -= indent;
        this.pair('OUTDENT');
        this.token('OUTDENT', indent, 0, outdentLength);
        if (oldDebt > 0 && indent + moveOut === oldDebt) {
          this.indebt = oldDebt;
          moveOut = 0;
          break;
        }
