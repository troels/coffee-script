...
class exports.Rewriter
  ...
  rewrite: (@tokens) ->
    ...
    @ostracizeBanishedTokens()
    @tokens

  # Certain tokens have been marked banished as they are in the best
  # case redundant and in the worst case will interfere with the main
  # parser.
  # Therefore we ostracize them.
  ostracizeBanishedTokens: () ->
    banishedTokens = (i for i in [0 ... @tokens.length] when @tokens[i].banish)
    for i in banishedTokens.reverse()
      @tokens.splice i, 1
  ...
  addImplicitBracesAndParens: ->
    ...
    @scanTokens (token, i, tokens) ->
      ...
      if tag in EXPRESSION_END
        while inImplicit()
          if inImplicitCall()
            ...
            stack.pop()
        token.banish = yes if stackTop()?[2]?.banish
        stack.pop()
      ...
      if @matchTags(i, ['RETURN', IMPLICIT_FUNC...], 'INDENT') and
         stackTop()?[0] isnt '[' and
        ...
        if tag isnt 'RETURN'
          startImplicitCall i + 1
          i += 1
        stack.push ['INDENT', i + 1, banish: yes]
        @tokens[i + 1].banish = yes
        return forward(2)
